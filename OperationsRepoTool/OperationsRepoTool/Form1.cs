    ...
    public partial class Form1 : Form
    {
        public Form1()
        {
            ...
            try
            {
                ...
                xmlPath_textBox.Text = iniDirectoryPath + "\\OperationsRepoTool.json";
        ...
        private async void button10_Click(object sender, EventArgs e)
        {
            ...
            if (dialogResult == DialogResult.Yes)
            {
                ...
                foreach (ListViewItem item in compareExcessFiles_listView.Items)
                {
                    ...
                    await Task.Run(() =>
                    {
                        File.Delete(repoConfigJson.syncFolder + item.Text.Split(':')[0]);
                    });
                ...
                foreach (ListViewItem item in compareMissingFiles_listView.Items)
                {
                    ...
                    CheckPath(repoConfigJson.syncFolder + item.Text.Split(':')[0]);

                    await Task.Run(() =>
                    {
                        File.Copy(repoConfigJson.modsFolder + item.Text.Split(':')[0], repoConfigJson.syncFolder + item.Text.Split(':')[0], true);
                    });
